...
/// 仅首个 Flutter 引擎注册的插件实例处理 `kAEGetURL`。desktop_multi_window 等会为子引擎再次调用
/// `RegisterGeneratedPlugins`，若此处每次都用「当前实例」注册 Apple Event，最后一次（子引擎）会顶掉主引擎，
/// 而 `_currentAuthorizationFlow` 仍在主实例上 → 回调永远无法 `resume`，authorize 永久挂起。
#if TARGET_OS_OSX
static FlutterAppauthPlugin *primaryMacOSOAuthPlugin;
#endif
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  ...
  // 供宿主 AppDelegate 在 `application:openURLs:` 中直接把重定向交给 AppAuth（`dispatchRawAppleEvent` 在部分系统上不可靠）。
  ...
  if (!primaryMacOSOAuthPlugin) {
    primaryMacOSOAuthPlugin = instance;
  }
  [appleEventManager setEventHandler:primaryMacOSOAuthPlugin
...
#if TARGET_OS_OSX
- (void)deliverOAuthRedirectURL:(NSURL *)url {
  if (!url) {
    return;
  }
  if ([_currentAuthorizationFlow resumeExternalUserAgentFlowWithURL:url]) {
