...
import 'package:just_audio_media_kit/dsd_playback_hints.dart';
...
class MediaKitPlayer extends AudioPlayerPlatform {
  ...
  MediaKitPlayer(super.id) {
    ...
    // 与 v5b 可出声配置一致：DSD 解码后再以 48kHz PCM 送 CoreAudio。
    if (Platform.isMacOS) {
      unawaited(setProperty(_player, 'audio-samplerate', '48000'));
      unawaited(setProperty(_player, 'audio-format', 's16'));
    }
  ...
  Future<LoadResponse> load(LoadRequest request) async {
    ...
    if (override is Duration) {
      _updateDuration(override);
      unawaited(setProperty(_player, 'loop-file', 'no'));
      final sec = override.inMicroseconds / 1000000.0;
      if (sec > 1) {
        unawaited(setProperty(_player, 'length', sec.toStringAsFixed(3)));
      }
    }
    _scheduleLoadReadyFallback();
  ...
  /// DSD 等格式在 libmpv 上可能长期不结束 buffering，导致 [load] 永不完成、UI 无响应。
  /// 勿将 [_duration] 置为 [Duration.zero]，否则 just_audio 会立刻进入 completed 并触发自动切歌。
  void _scheduleLoadReadyFallback() {
    unawaited(Future<void>.delayed(const Duration(milliseconds: 1500), () {
      ...
      completer.complete(_duration);
      _updatePlaybackEvent();
      _logger.fine('load ready fallback (mpv did not report buffering end)');
    }));
  }
  ...
  Media _convertAudioSourceIntoMediaKit(AudioSourceMessage audioSource) {
    switch (audioSource) {
      case final UriAudioSourceMessage uriSource:
        final pathLower = uriSource.uri.toLowerCase();
        Duration? overrideDuration;
        if (pathLower.endsWith('.dsf') || pathLower.endsWith('.dff')) {
          overrideDuration =
              DsdPlaybackHints.durationForFileUri(uriSource.uri);
        }
        return Media(
          uriSource.uri,
          httpHeaders: audioSource.headers,
          extras: overrideDuration != null
