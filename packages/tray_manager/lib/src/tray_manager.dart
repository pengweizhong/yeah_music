...
class TrayManager {
  ...
  /// Asset path under `data/flutter_assets`, or an absolute filesystem path.
  static String _resolveTrayIconPath(String iconPath) {
  ...
  Future<void> setIcon(
    ...
    final Map<String, dynamic> arguments = {
      'id': shortid.generate(),
      'iconPath': _resolveTrayIconPath(iconPath),
  ...
  Future<void> setContextMenu(Menu menu) async {
    _menu = menu;
    final Map<String, dynamic> arguments = {
      'menu': _encodeMenuForPlatform(menu),
    };
    await _channel.invokeMethod('setContextMenu', arguments);
  }

  Map<String, dynamic> _encodeMenuForPlatform(Menu menu) {
    if (defaultTargetPlatform == TargetPlatform.macOS) {
      return menu.toJson();
    }
    return {
      'items': menu.items?.map(_encodeMenuItemForPlatform).toList(),
    };
  }

  Map<String, dynamic> _encodeMenuItemForPlatform(MenuItem item) {
    final json = item.toJson();
    final icon = item.icon;
    if (icon != null && icon.isNotEmpty && !kIsWeb) {
      json['icon'] = path.isAbsolute(icon)
          ? icon
          : path.normalize(path.joinAll([
              path.dirname(Platform.resolvedExecutable),
              'data',
              'flutter_assets',
              icon,
    ...
    if (submenu != null) {
      json['submenu'] = _encodeMenuForPlatform(submenu);
    }
    return json;
  }
