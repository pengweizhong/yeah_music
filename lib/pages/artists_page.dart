...
import 'package:yeah_music/utils/artist_library_index.dart';
...
/// 艺术家列表页：按拆分后的艺人聚合；合唱单独成行，单人下列出其参与的所有曲目（含合唱）。
class ArtistsBrowserPage extends StatefulWidget {
  const ArtistsBrowserPage({super.key});

  @override
  State<ArtistsBrowserPage> createState() => _ArtistsBrowserPageState();
}

class _ArtistsBrowserPageState extends State<ArtistsBrowserPage> {
  @override
  void initState() {
    super.initState();
  ...
  void _openArtistSearch(BuildContext context, List<ArtistIndexRow> rows) {
    // 使用独立路由 + TextField，避免 showSearch/SearchDelegate 在部分平台触发 HardwareKeyboard 断言导致无法输入。
    Navigator.of(context).push<void>(
      MaterialPageRoute<void>(
        builder: (_) => ArtistsBrowserSearchPage(allRows: rows),
      ),
    );
  }
    ...
    return SongPlaylistThemedScaffold(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              ...
              final rows = ArtistLibraryIndex.buildRows(
                playList.libraryMergedSongs,
                l10n,
              );
              _openArtistSearch(context, rows);
            },
          ),
        ],
      ),
      body: Consumer<PlayListProvider>(
        builder: (context, playList, _) {
          ...
          final rows = ArtistLibraryIndex.buildRows(merged, l10n);
          return SongPlaylistBodyUnderlapColumn(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final isCollab = isCollabArtistEntryKey(row.entryKey);
                return ListTile(
                  contentPadding:
                      const EdgeInsets.symmetric(horizontal: 16, vertical: 2),
                  leading: LibraryIndexCoverLeading(
                    coverBytes: row.coverBytes,
                    fallbackIcon: isCollab
                    ...
                    Navigator.push<void>(
                      ...
                      MaterialPageRoute<void>(
                        builder: (_) => ArtistSongsPage(
                          artistEntryKey: row.entryKey,
                          artistDisplayTitle: row.title,
                        ),
                      ),
                    );
                  },
...
/// 独立搜索页：不用 [SearchDelegate]，避免部分平台（尤其 macOS）上 `HardwareKeyboard` 与搜索 overlay 冲突导致搜索框无法输入。
class ArtistsBrowserSearchPage extends StatefulWidget {
  const ArtistsBrowserSearchPage({super.key, required this.allRows});

  final List<ArtistIndexRow> allRows;

  @override
  State<ArtistsBrowserSearchPage> createState() => _ArtistsBrowserSearchPageState();
}

class _ArtistsBrowserSearchPageState extends State<ArtistsBrowserSearchPage> {
      ...
      appBar: AppBar(
        ...
        title: TextField(
          ...
          decoration: InputDecoration(
            hintText: l10n.artistsSearchHint,
...
class ArtistSongsPage extends StatefulWidget {
  const ArtistSongsPage({
    super.key,
    required this.artistEntryKey,
    required this.artistDisplayTitle,
  });

  final String artistEntryKey;
  final String artistDisplayTitle;

  @override
  State<ArtistSongsPage> createState() => _ArtistSongsPageState();
}

class _ArtistSongsPageState extends State<ArtistSongsPage> with RouteAware {
  ...
  void didPopNext() {
    if (!mounted) return;
    final playList = context.read<PlayListProvider>();
    if (!playList.playbackSessionIsLibraryByArtist) return;
    final songs = ArtistLibraryIndex.songsForEntry(
      widget.artistEntryKey,
      playList.libraryMergedSongs,
    );
  ...
  List<Song> _songsForArtist(List<Song> merged) {
    final picked = ArtistLibraryIndex.songsForEntry(
      widget.artistEntryKey,
      merged,
    );
    return sortSongsCopy(picked, SongListSortType.name, true);
  }
  ...
  void _openSearch(
    ...
    showSearch(
      ...
      delegate: SongSearchDelegate(
        ...
        playbackQueueSession: PlaybackSessionSurface.libraryByArtist,
    ...
    return SongPlaylistThemedScaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              final playList = context.read<PlayListProvider>();
              final q = _songsForArtist(playList.libraryMergedSongs);
      ...
      body: Consumer<PlayListProvider>(
        builder: (context, playList, _) {
          ...
          final songs = _songsForArtist(playList.libraryMergedSongs);
          if (songs.isEmpty) {
            return SongPlaylistBodyUnderlapColumn(
              child: Center(
                child: Text(
            ...
            child: SongPlaylistSongListView(
              ...
              itemBuilder: (context, song, index, isCurrent) {
                return CompactSongListRow(
                  key: ValueKey<String>(
                    'artist_${index}_${normSongPath(song.path)}',
                  ),
                  song: song,
                  ...
                  onTap: () async {
                    ...
                    await playList.setPlaybackQueueAndPlay(
                      ...
                      index,
                      session: PlaybackSessionSurface.libraryByArtist,
                    );
                  },
                );
              },
