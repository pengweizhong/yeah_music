...
class AppLocalizationsJa extends AppLocalizations {
  ...
  @override
  String get settingsAboutDialogPrivacy => 'プライバシー';
  ...
  String get settingsPrivacyPolicyBody =>
      'このプライバシーポリシーは Yeah Music に適用されます。Yeah Music はオープンソースで無料の、ローカル優先の音楽プレーヤーです。Yeah Music は開発者が運営するアカウントシステムを提供せず、広告配信サービスを運営せず、広告プロファイルやユーザー行動分析のための第三者分析 SDK を組み込まず、個人情報を販売しません。\n\n1. ローカル音楽とメディアライブラリ\nシステム権限を付与したり音楽フォルダを選択したりすると、Yeah Music はローカルの音声ファイル、ファイル名、ファイルパス、音声タグ、アートワーク、歌詞、プレイリスト、再生履歴を読み取り、ライブラリ表示、再生、タグ編集、再生回数の集計、再生位置の復元、プレイリスト作成に使用します。これらのデータは既定では端末内に保存され、Yeah Music が開発者運営サーバーへ自動送信することはありません。\n\n2. OneDrive 機能\nOneDrive のサインイン、クラウドライブラリ、同期、アップロード、ダウンロードを有効にすると、Yeah Music は Microsoft OneDrive と通信します。アクセストークンは端末の安全なストレージに保存されます。ライブラリ索引、同期データ、アップロードファイル、ダウンロードファイルは、ユーザーの操作に応じて OneDrive とやり取りされます。Microsoft サービスでのデータ処理は Microsoft の規約とプライバシーポリシーに従います。\n\n3. 音楽認識\n音楽認識を使用すると、Yeah Music は短い音声サンプルを録音し、選択または設定した認識サービス（AudD、ACRCloud など）へ送信して照合します。認識結果には曲名、アーティスト、アルバムなどの情報が含まれる場合があります。第三者サービスによる音声処理を望まない場面ではこの機能を使用しないでください。\n\n4. 診断ログ\n診断ログは既定でオフです。有効にすると、Yeah Music は実行ログを端末内に保存します。Android ではヘッドセット／メディアボタンイベントも記録します。ログにはサイズ上限があり、設定で表示、コピー、共有、消去できます。共有前に、ファイルパス、ファイル名、エラー詳細など公開したくない内容が含まれていないか確認してください。\n\n5. 更新確認と外部リンク\nアプリについて画面でバージョン番号をタップして更新を確認すると、Yeah Music は GitHub Release 情報にアクセスします。リポジトリ、スポンサー、その他の外部リンクはブラウザまたはシステムで開きます。外部サイトや第三者サービスのデータ処理は、それぞれのプライバシーポリシーに従います。\n\n6. 権限の利用\nYeah Music はストレージ／メディアライブラリ、マイク、通知、バックグラウンドメディア再生、ネットワークなどの権限を要求する場合があります。これらはローカル音楽の再生・管理、認識用音声の録音、再生通知の表示、ヘッドセットやロック画面操作の処理、OneDrive へのアクセス、更新確認など、対応する機能にのみ使用されます。システム設定で権限を取り消せますが、関連機能は利用できなくなる場合があります。\n\n7. データの保持と削除\nローカルライブラリ索引、再生履歴、プレイリスト、アプリ設定、OneDrive 設定、診断ログは、アプリ内で削除、サインアウト、関連機能の無効化または消去を行うまで、またはアプリをアンインストールするまで端末内に保存されます。アンインストールすると通常はアプリ専用領域のデータが削除されますが、ユーザーが選択した音楽ファイル、公開フォルダにダウンロードしたファイル、エクスポートしたファイルは削除されません。\n\n8. 子どものプライバシー\nYeah Music は子ども向けの専用サービスではなく、子どもの個人情報を意図的に収集しません。未成年者は、ネットワーク、録音、クラウドサービスを伴う機能を保護者の同意と指導のもとで使用してください。\n\n9. 変更と連絡先\nYeah Music は機能変更に応じて本ポリシーを更新する場合があります。重要な変更はアプリ更新またはリリースノートで通知されることがあります。プライバシーやデータ処理に関する質問は、GitHub リポジトリ https://github.com/pengweizhong/yeah_music の Issues から連絡できます。';
