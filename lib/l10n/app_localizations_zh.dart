...
class AppLocalizationsZh extends AppLocalizations {
  ...
  @override
  String get settingsAboutDialogPrivacy => '隐私';

  @override
  String get settingsPrivacyPolicyTitle => '隐私协议';

  @override
  String get settingsPrivacyPolicyBody =>
      '本隐私协议适用于 Yeah Music。Yeah Music 是一款开源免费的本地优先音乐播放器。Yeah Music 不提供开发者自建账号系统，不运营广告投放服务，不集成用于广告画像或用户行为统计的第三方分析 SDK，也不会出售你的个人信息。\n\n1. 本地音乐与媒体库\n在你授权系统权限或主动选择音乐目录后，Yeah Music 会读取本机音频文件、文件名、文件路径、音频标签、封面、歌词、播放列表和播放记录，用于展示曲库、播放音乐、编辑标签、统计播放次数、恢复播放进度和生成歌单。这些数据默认保存在你的设备上，Yeah Music 不会主动上传到开发者服务器。\n\n2. OneDrive 功能\n当你启用 OneDrive 登录、云端曲库、同步、上传或下载时，Yeah Music 会与 Microsoft OneDrive 服务通信。访问令牌会保存在设备安全存储中；曲库索引、同步片段、上传文件和下载文件只会根据你的操作与 OneDrive 交互。Microsoft 服务对数据的处理以 Microsoft 的服务条款和隐私政策为准。\n\n3. 听歌识曲\n当你使用听歌识曲时，Yeah Music 会录制短音频片段，并发送给你选择或配置的识别服务（如 AudD 或 ACRCloud）进行匹配。识别结果可能包含歌曲名、歌手、专辑等信息。请不要在不希望音频被第三方服务处理的场景中使用该功能。\n\n4. 诊断日志\n诊断日志默认关闭。开启后，Yeah Music 会在本机保存运行日志；Android 还会记录耳机线控 / 媒体按键事件，用于排查问题。日志有大小上限，可在设置中查看、复制、分享或清空。分享日志前请自行确认其中是否包含文件路径、文件名、错误信息或其他你不希望公开的内容。\n\n5. 更新检查与外部链接\n当你在关于页面点击版本号检查更新时，Yeah Music 会访问 GitHub Release 信息。仓库、赞助与其他外部链接会交由浏览器或系统组件打开。外部网站或第三方服务的数据处理规则由其各自的隐私政策约束。\n\n6. 权限使用\nYeah Music 可能请求存储 / 媒体库、麦克风、通知、后台媒体播放、网络等权限。这些权限仅用于对应功能：播放和管理本地音乐、录制识曲片段、显示播放通知、处理耳机和锁屏控制、访问 OneDrive、检查更新等。你可以在系统设置中撤销权限，但相关功能可能无法继续使用。\n\n7. 数据保留与删除\n本地曲库索引、播放记录、歌单、应用设置、OneDrive 配置和诊断日志会保存在你的设备上，直到你在应用中清理、退出账号、关闭或清空相关功能，或卸载应用。卸载应用通常会移除应用私有目录中的数据，但你主动选择的音乐文件、下载到公共目录的文件和导出的文件不会因此删除。\n\n8. 儿童隐私\nYeah Music 不面向儿童提供专门服务，也不会故意收集儿童个人信息。未成年人应在监护人同意和指导下使用涉及网络、录音或云服务的功能。\n\n9. 协议变更与联系\nYeah Music 可能随功能变化更新本协议。重大变更会随应用更新或发布说明告知。若你对隐私或数据处理有疑问，可通过 GitHub 仓库 https://github.com/pengweizhong/yeah_music 的 Issues 联系。';
...
class AppLocalizationsZhHant extends AppLocalizationsZh {
  ...
  @override
  String get settingsAboutDialogPrivacy => '隱私';
  ...
  @override
  String get settingsPrivacyPolicyBody =>
      '本隱私權政策適用於 Yeah Music。Yeah Music 是一款開源免費、本機優先的音樂播放器。Yeah Music 不提供開發者自建帳號系統，不營運廣告投放服務，不整合用於廣告画像或使用者行為統計的第三方分析 SDK，也不會出售你的個人資訊。\n\n1. 本機音樂與媒體庫\n在你授權系統權限或主動選擇音樂目錄後，Yeah Music 會讀取本機音訊檔案、檔名、檔案路徑、音訊標籤、封面、歌詞、播放列表和播放記錄，用於顯示曲庫、播放音樂、編輯標籤、統計播放次數、恢復播放進度和建立歌單。這些資料預設保存在你的裝置上，Yeah Music 不會主動上傳到開發者伺服器。\n\n2. OneDrive 功能\n當你啟用 OneDrive 登入、雲端曲庫、同步、上傳或下載時，Yeah Music 會與 Microsoft OneDrive 服務通訊。存取權杖會保存在裝置的安全儲存中；曲庫索引、同步片段、上傳檔案和下載檔案只會依你的操作與 OneDrive 互動。Microsoft 服務對資料的處理以 Microsoft 的服務條款和隱私權政策為準。\n\n3. 聽歌識曲\n當你使用聽歌識曲時，Yeah Music 會錄製短音訊片段，並傳送給你選擇或設定的識別服務（如 AudD 或 ACRCloud）進行比對。識別結果可能包含歌曲名、歌手、專輯等資訊。請不要在不希望音訊被第三方服務處理的場景中使用該功能。\n\n4. 診斷日誌\n診斷日誌預設關閉。開啟後，Yeah Music 會在本機保存執行日誌；Android 還會記錄耳機線控 / 媒體按鍵事件，用於排查問題。日誌有大小上限，可在設定中查看、複製、分享或清空。分享日誌前請自行確認其中是否包含檔案路徑、檔名、錯誤資訊或其他你不希望公開的內容。\n\n5. 更新檢查與外部連結\n當你在關於頁面點擊版本號檢查更新時，Yeah Music 會存取 GitHub Release 資訊。存放庫、贊助與其他外部連結會交由瀏覽器或系統元件開啟。外部網站或第三方服務的資料處理規則由其各自的隱私權政策約束。\n\n6. 權限使用\nYeah Music 可能請求儲存 / 媒體庫、麥克風、通知、背景媒體播放、網路等權限。這些權限僅用於對應功能：播放和管理本機音樂、錄製識曲片段、顯示播放通知、處理耳機和鎖屏控制、存取 OneDrive、檢查更新等。你可以在系統設定中撤銷權限，但相關功能可能無法繼續使用。\n\n7. 資料保留與刪除\n本機曲庫索引、播放記錄、歌單、應用程式設定、OneDrive 配置和診斷日誌會保存在你的裝置上，直到你在應用程式中清理、登出帳號、關閉或清空相關功能，或解除安裝應用程式。解除安裝應用程式通常會移除應用程式私有目錄中的資料，但你主動選擇的音樂檔案、下載到公共目錄的檔案和匯出的檔案不會因此刪除。\n\n8. 兒童隱私\nYeah Music 不面向兒童提供專門服務，也不會故意收集兒童個人資訊。未成年人應在監護人同意和指導下使用涉及網路、錄音或雲端服務的功能。\n\n9. 協議變更與聯絡\nYeah Music 可能隨功能變化更新本政策。重大變更會隨應用程式更新或發行說明告知。若你對隱私或資料處理有疑問，可透過 GitHub 存放庫 https://github.com/pengweizhong/yeah_music 的 Issues 聯絡。';
